...
const ManageStaffMechanical = () => {
   ...
   const { data } = useSWR(
      `/api/users?page=${pageIndex}&role=mechanical&name=${searchValue}&email=${searchValue}`
   )
   ...
   const handleSubmitFormik = async (values, actions) => {
      ...
      try {
         if (isAdd) {
            await UserService.createUser({ ...values, role: 'mechanical' })
         } else {
   ...
   return (
      ...
      >
         ...
         <AlertDialogComponent
            header='Hapus staff'
            body='Menghapus staff mekanik akan menghapus seluruh data yang ketergantungan dengan ID ini, Yakin ingin menghapus?'
            isOpen={isOpenAlert}
            onClose={onCloseAlert}
...
export default ManageStaffMechanical
